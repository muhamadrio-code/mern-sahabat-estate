      ...
      <div className='flex w-full max-w-6xl justify-between items-center px-3'>
        ...
        <nav className='flex items-center gap-2'>
          ...
          <Link to={currentUser ? "/profile" : "/signin"} className='ml-2'>{
            currentUser ? <img src={currentUser.avatar} alt="profile_picture.jpg" className='w-8 h-8 border-2 border-slate-300 rounded-full'/> : <button className='w-full h-full px-4 py-2 font-semibold bg-primary rounded-lg text-white hover:no-underline hover:bg-primary-50'>Sign In</button>
          }</Link>
        </nav>
      </div>
    </header>
